...
const Title = styled.h3`
  ...
  font-size: 16px;
  color: ${props =>
    props.theme.headerTextColor ? props.theme.headerTextColor : "white"};
  font-family: inherit;
  font-weight: 500;
  line-height: 1.1;
...
export default class PanelTitle extends Component {
  ...
  render() {
    ...
    if (collapse) {
      elem = <TitleCollapse onClick={clickFunc}>{children}</TitleCollapse>
    } else {
      elem = children
    }
    return <Title style={{ ...style }}>{elem}</Title>
  }
}
